...
#define ADC_CONVERTED_VALUE_NUM 4
__IO uint16_t ADC_ConvertedValue[ADC_CONVERTED_VALUE_NUM];
 ...
 */
 ...
 {
	if(length > ADC_CONVERTED_VALUE_NUM)
	{
		length = ADC_CONVERTED_VALUE_NUM;
	}
	for(uint8_t i = 0; i < length; ++i)
	{
		value_arrays[i] = ADC_ConvertedValue[i];
	}
 }
