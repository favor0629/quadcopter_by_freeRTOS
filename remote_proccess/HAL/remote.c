...
volatile Remote_t Remote __attribute__((aligned(4))) = {1000,1000,1000,1000,1000,1000,1000,1000};
...
#define NRF_FRAME_LEN   32
#define NRF_CHECKSUM_IDX 31
uint8_t tx_data[NRF_FRAME_LEN] = {0xAA,0xAF,0x03,32-5};//匿名通信协议
void NRF_SEND(void)
{
	Remote_t snapshot;
	...
	__disable_irq(); // 禁止中断
	snapshot = Remote; // 复制Remote数据到snapshot
	__enable_irq(); // 允许中断

	tx_data[0] = 0xAA; // 起始字节
	tx_data[1] = 0xAF; // 命令字节
	tx_data[2] = 0x03;
	tx_data[3] = 32 - 5; // 数据长度（总长度 - 头部和校验字节）

	tx_data[4]  = (uint8_t)(snapshot.roll  >> 8);
    tx_data[5]  = (uint8_t)(snapshot.roll  & 0xFF);

    tx_data[6]  = (uint8_t)(snapshot.pitch >> 8);
    tx_data[7]  = (uint8_t)(snapshot.pitch & 0xFF);

    tx_data[8]  = (uint8_t)(snapshot.thr   >> 8);
    tx_data[9]  = (uint8_t)(snapshot.thr   & 0xFF);

    tx_data[10] = (uint8_t)(snapshot.yaw   >> 8);
    tx_data[11] = (uint8_t)(snapshot.yaw   & 0xFF);

    tx_data[12] = (uint8_t)(snapshot.AUX1  >> 8);
    tx_data[13] = (uint8_t)(snapshot.AUX1  & 0xFF);

    tx_data[14] = (uint8_t)(snapshot.AUX2  >> 8);
    tx_data[15] = (uint8_t)(snapshot.AUX2  & 0xFF);

    tx_data[16] = (uint8_t)(snapshot.AUX3  >> 8);
    tx_data[17] = (uint8_t)(snapshot.AUX3  & 0xFF);

    tx_data[18] = (uint8_t)(snapshot.AUX4  >> 8);
    tx_data[19] = (uint8_t)(snapshot.AUX4  & 0xFF);

	// 20-30字节是保留，这里赋值为0，31字节是校验位
	...
	tx_data[NRF_CHECKSUM_IDX] = checksum; // 校验位放在最后

	NRF24L01_TxPacket(tx_data); //调用NRF发射数据
...
void RC_Analy(void) //10ms调用一次
{
	...
	uint16_t adc_convert_values[4];
	ADC1_DMA_Get_value(adc_convert_values, 4);
	//ADC_ConvertedValue  DMA自动回读的值，直接可以用，无需手动读取ADC转换结果
	Remote.thr = (uint16_t)(0.20f*adc_convert_values[1])+1000 + offset.thr;//油门
	last_thr = Remote.thr = Remote.thr*0.25f + 0.75f*last_thr;
	Remote.pitch 	= 	1000 + (uint16_t)(0.25f*adc_convert_values[2]) + offset.pitch;//
	last_pitch = Remote.pitch = Remote.pitch*0.25f + 0.75f*last_pitch;
	Remote.yaw 		= 	1000 + (uint16_t)(0.25f*adc_convert_values[0]) + offset.yaw;//方向
	last_yaw = Remote.yaw = Remote.yaw*0.25f + 0.75f*last_yaw;
	Remote.roll 	=	1000 +(uint16_t)(0.25f*adc_convert_values[3]) + offset.roll;//副翼
