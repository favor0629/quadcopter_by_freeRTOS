 ...
 * @return     
 ***************************************************************/	
void CascadePID(PidObject* inter_ring,PidObject* outer_ring,const float dt)  //串级PID
{	 
	pidUpdate(outer_ring,dt);    //先计算外环
	inter_ring->desired = outer_ring->out;
	pidUpdate(inter_ring,dt);    //再计算内环	
}
