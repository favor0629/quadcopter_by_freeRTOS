...
/**
  ...
  * 使用递归互斥锁以支持函数间的互相调用
  */
static SemaphoreHandle_t xOLED_Mutex = NULL;
...
void OLED_GPIO_Init(void)
{
	...
	/*创建FreeRTOS互斥锁（如果还未创建）*/
	if (xOLED_Mutex == NULL)
	{
		xOLED_Mutex = xSemaphoreCreateRecursiveMutex();
	}
}
...
void OLED_WriteCommand(uint8_t Command)
{
	...
	if (xOLED_Mutex != NULL)
	{
		xSemaphoreTakeRecursive(xOLED_Mutex, portMAX_DELAY);
	}
	...
	if (xOLED_Mutex != NULL)
	{
		xSemaphoreGiveRecursive(xOLED_Mutex);
	}
}
